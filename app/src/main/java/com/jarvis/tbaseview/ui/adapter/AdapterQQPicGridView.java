...
public class AdapterQQPicGridView extends BaseAdapter {
	...
	public AdapterQQPicGridView(Context context,ArrayList<String> list,int maxNumber,int numColumns) {
	...
	@Override
	public int getCount() {
		return (list != null && list.size() < maxNumber) ? list.size() + 1 : maxNumber + 1;
	}
...
//			albumRes.disPlayBitmap(context, holderView.icon, null, list.get(position));
			bitmapHelp.display(holderView.icon,list.get(position));
		}
		return convertView;
	}
