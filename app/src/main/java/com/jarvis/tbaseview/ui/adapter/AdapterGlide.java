...
public class AdapterGlide extends BaseAdapter {
	...
	public AdapterGlide(Context context) {
		...
		addUrl();
		glideHelp=new GlideHelp(context);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView==null) {
			...
			convertView=LayoutInflater.from(context).inflate(R.layout.adapter_glide, null);
			holderView.item=(ImageView) convertView.findViewById(R.id.adapter_glide_icon);
			convertView.setTag(holderView);
		}else {
			holderView=(HolderView) convertView.getTag();
		}
		glideHelp.display(list.get(position),holderView.item);
		return convertView;
	}
