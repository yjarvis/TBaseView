...
package com.jarvis.tbaseview.support.fresco.config;
...
import com.jarvis.tbaseview.support.fresco.instrumentation.InstrumentedDraweeView;
...
public class ConfigConstants {
  ...
  private static final String IMAGE_PIPELINE_SMALL_CACHE_DIR = "TBaseView_FrescoCache";//小图所放路径的文件夹名
  private static final String IMAGE_PIPELINE_CACHE_DIR = "TBaseView_FrescoCache";//默认图所放路径的文件夹名
...
//			  .setMinDecodeIntervalMs(intervalMs)//最小解码间隔（分位单位）
  ...
  /**图片显示
   * @param view InstrumentedDraweeView
   * @param uri
   * @return
   */
...
//			  .reset()//重置
			  .setAutoPlayAnimations(true)//自动播放图片动画
//			  .setCallerContext(callerContext)//回调
			  .setControllerListener(new BaseControllerListener<Object>() {})//监听图片下载完毕等
//			  .setDataSourceSupplier(dataSourceSupplier)//数据源
//			  .setFirstAvailableImageRequests(firstAvailableImageRequests)//本地图片复用，可加入ImageRequest数组
			  .setImageRequest(imageRequest)//设置单个图片请求～～～不可与setFirstAvailableImageRequests共用，配合setLowResImageRequest为高分辨率的图
//			  .setLowResImageRequest(ImageRequest.fromUri(lowResUri))//先下载显示低分辨率的图
...
public static void init(final Resources resources) {
	    if (sPlaceholderDrawable == null) {
	      sPlaceholderDrawable = resources.getDrawable(R.color.graylight);
	    }
	    if (sErrorDrawable == null) {
	      sErrorDrawable = resources.getDrawable(R.color.bluelight);
	    }
	  }
